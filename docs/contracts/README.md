...
- `backend/app/schemas/auth.py`: login/refresh/logout input, fresh Principal, demo identity, and scoped employee identity responses.
...
Domain prefixes additionally include `org`, `pos`, `sid`, and `audit` for organization, position, authentication session, and security audit records. Existing prefixes remain unchanged. Trace and request identifiers use `trc_<ULID>` and `req_<ULID>`. Idempotency keys use a separate caller-owned `idem_...` format and have different lifecycle semantics.
...
- The contract package and current `schema_version` are `2.2.0`; `2.0.0` and `2.1.0` remain supported compatible versions.
...
The directly developed Web and Android applications consume contract version `2.2.0`. `packages/types/src/index.ts` exports generated `Contract*` types. Web, Android, shared rules, Mock data, and tests use formal `snake_case` fields, `<prefix>_<ULID>` IDs, and separate TB/LR/AP/AR state domains directly.
