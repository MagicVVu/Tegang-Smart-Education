...
| C-02 Windows/Docker 基线 | Completed | 四服务、健康检查、持久化、真实模型连通性和中文/空格目录复现均已通过；主机 Node/JDK 偏低作为已知限制保留 |
...
- C-02 最小 FastAPI 只负责健康与依赖检查，不等同于正式业务后端。
